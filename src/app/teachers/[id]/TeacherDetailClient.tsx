          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-4 gap-md mt-lg">
              {[
                ...
                { label: 'Tổng học viên', value: teacher.currentClasses.reduce((sum: number, c: any) => sum + (c.enrolled || 0), 0) },
        ...
        <div className="p-lg">
              ...
              <div className="space-y-lg">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-md">
                    ...
                    <div className="bg-surface-container-low rounded-xl p-md">
                      ...
                      <div className="flex flex-wrap gap-xs">
                        {teacher.certificates?.length > 0 ? teacher.certificates.map((c: string) => (
                          <span key={c} className="text-label-sm bg-primary/10 text-primary px-sm py-xs rounded-full">{c}</span>
                        )) : (
                          <span className="text-label-sm text-on-surface-variant">Chưa có chứng chỉ</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <div className="mt-md bg-surface-container-low rounded-xl p-md">
                    ...
                    <div className="flex flex-wrap gap-xs">
                      {teacher.teachingStrengths?.length > 0 ? teacher.teachingStrengths.map((s: string) => (
                        ...
                        <span className="text-label-sm text-on-surface-variant">Chưa cập nhật thế mạnh</span>
                      )}
                    </div>
                  </div>
                </div>
                    ...
                    {[
                      ...
                      <div key={info.label} className="flex items-start gap-sm bg-surface-container-low rounded-xl p-md">
                        ...
                        <div>
                          ...
                          <p className="text-body-md text-on-surface">{info.value || 'Chưa có'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'schedule' && (
            <div className="animate-fade-in overflow-x-auto">
              <h3 className="text-label-sm text-on-surface-variant uppercase tracking-wider mb-md">Lịch dạy tuần này (Dữ liệu mẫu)</h3>
          ...
          {activeTab === 'classes' && (
            <div className="animate-fade-in">
              <h3 className="text-label-sm text-on-surface-variant uppercase tracking-wider mb-md">Danh sách lớp hiện tại (Dữ liệu mẫu)</h3>
                    ...
                    {teacher.currentClasses.length > 0 ? teacher.currentClasses.map((cls: any) => (
                      <tr key={cls.code} className="hover:bg-primary/[0.02] transition-colors">
                        <td className="py-md px-md">
                          <div className={`inline-flex items-center gap-sm px-sm py-xs rounded-lg border ${COLOR_MAP[cls.colorKey] || COLOR_MAP['primary']}`}>
