...
export default function StudentDetailPage({ params }: Props) {
  // Using React.use() pattern — this is a client component with async params
  const [activeTab, setActiveTab] = useState('overview');
  const [resolvedParams, setResolvedParams] = useState<{ id: string } | null>(null);

  if (!resolvedParams) {
    params.then(setResolvedParams);
    // Render with id=1 as placeholder during resolution
  }

  const id = resolvedParams?.id ?? '1';
  const student = getStudentById(id);
  if (!student && resolvedParams) return notFound();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-md">
        ...
        <div className="flex flex-col gap-md">
          ...
          <div className="card p-lg flex flex-col items-center text-center">
            ...
            <div className="w-full mt-lg flex gap-sm">
              <button className="flex-1 btn-secondary justify-center text-[13px]">
        ...
        <div className="flex flex-col gap-md">
          ...
          <div className="grid grid-cols-3 gap-md">
            {[
              { label: 'Lớp đang học', value: student.enrolledCourses.filter(c => c.status === 'Đang học').length, icon: 'class', sub: primaryCourse?.className ?? '—' },
                    ...
                    <div className="grid grid-cols-2 gap-md">
                      {[
                        ...
                        <div key={field.label} className="bg-surface-container-low rounded-xl p-md">
                          ...
                          <p className="text-body-md text-on-surface font-medium mt-xs">{field.value}</p>
                        </div>
                      ))}
                    </div>
                  </div>
